...
public class AartiViewPager extends ViewPager{
	...
	public AartiViewPager(Context context) {
		this(context, null);
	}

	@SuppressWarnings("incomplete-switch")
	public AartiViewPager(Context context, AttributeSet attrs) {
		super(context, attrs);
		setClipChildren(false);
		// now style everything!
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AartiViewPager);
		int effect = ta.getInt(R.styleable.AartiViewPager_style, 0);
		String[] transitions = getResources().getStringArray(R.array.viewpager_effects);
		setTransitionEffect(TransitionEffect.valueOf(transitions[effect]));
		setFadeEnabled(ta.getBoolean(R.styleable.AartiViewPager_fadeEnabled, false));
		setOutlineEnabled(ta.getBoolean(R.styleable.AartiViewPager_outlineEnabled, false));
		setOutlineColor(ta.getColor(R.styleable.AartiViewPager_outlineColor, Color.WHITE));
		switch (mEffect) {
		case Stack:
		case ZoomOut:
			setFadeEnabled(true);
